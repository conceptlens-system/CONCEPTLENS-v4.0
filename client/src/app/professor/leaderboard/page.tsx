...
import { Trophy, Medal, Star, Crown, Users, TrendingUp, TrendingDown, Minus } from "lucide-react"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { fetchClassLeaderboard, fetchClasses } from "@/lib/api"
import { toast } from "sonner"

export default function ProfessorLeaderboardPage() {
    ...
    useEffect(() => {
        ...
        loadClassLeaders()
    }, [selectedClass, session?.user?.email])
            ...
            <Card className="border-dashed bg-slate-50/50 mt-4">
                <CardContent className="flex flex-col items-center justify-center py-12 text-slate-500">
                    ...
                    <p className="text-sm text-slate-400 text-center max-w-sm mt-2">
                        Students must complete exams to earn XP and appear on the leaderboard.
                    </p>
                </CardContent>
            </Card>
        )
                    ...
                    <div className="divide-y divide-slate-100">
                        {data.map((student, idx) => {
                            ...
                            return (
                                <div
                                    key={student._id}
                                    className="p-4 flex items-center justify-between transition-colors hover:bg-slate-50"
                                >
                                    <div className="flex items-center gap-6 sm:gap-10 w-1/2">
        ...
        <PageTransition className="space-y-8 pb-12 max-w-5xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-2 text-lg">View the top performing students in your classes.</p>
                </div>
            </div>
                ...
                <CardHeader className="px-0 pt-0 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div>
                        ...
                        <CardDescription className="text-base text-slate-500">
                            Select a class to see student rankings.
                        </CardDescription>
                    </div>
                    {classes.length > 0 && (
                ...
                {classes.length === 0 && !loading ? (
                    <Card className="border-dashed bg-slate-50/50 mt-4">
                        <CardContent className="flex flex-col items-center justify-center py-12 text-slate-500">
                            ...
                            <p className="text-sm text-slate-400 text-center max-w-sm mt-2">
                                You need to create a class first to view its leaderboard.
                            </p>
                        </CardContent>
                    </Card>
                ) : (
